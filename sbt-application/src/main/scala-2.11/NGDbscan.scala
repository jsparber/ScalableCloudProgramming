...
object ngDBSCAN {
  ...
  def exec(sc: SparkContext, records: RDD[Record]) {
    ...
    val emptyEdges:  org.apache.spark.rdd.RDD[org.apache.spark.graphx.Edge[Double]] = sc.parallelize(Seq())
    ...
    val randomEdges:  org.apache.spark.rdd.RDD[org.apache.spark.graphx.Edge[Double]] = 
    nodes.flatMap(n1 => Random.shuffle(collection.toList).take(k).map(n2 => Edge(n1._1, n2._1, calculateDistance(n1._2, n2._2))))
    ...
    while (!terminate) {
      ...
      val neighbors = nGraph.collectNeighborIds(EdgeDirection.Out)
      println("Number of neighbors")
      neighbors.collect.map(x=> x._1.toString + " " + x._2.length.toString).foreach(println)
      val xEdges = neighbors.flatMap(n1 => {
          //n1._2.flatMap(n2 => n1._2.map(n3 => Edge(n3._1, n2._1, calculateDistance(n3._2, n2._2))))})
        n1._2.flatMap(n2 => n1._2.map(n3 => Edge(n3, n2, 10.0)))}).distinct()
      ...
      println("NewEdges : " + newEdges.count)
      epsGraph = Graph(epsGraph.vertices, (epsGraph.edges ++ newEdges).distinct() )
      // Shrink nGraph
      val nodesToRemove = epsGraph.collectNeighborIds(EdgeDirection.Out)
      .filter(x => x._2.length >= Mmax).map(_._1).collect
      //FIXME: we propabily should filter edges as well
      val numberOfNodes = nGraph.vertices.count
      println("list of nodes to remove")
      nodesToRemove.foreach(println)

      println("list of current nodes")
      nGraph.vertices.map(_._1).foreach(println)
      nGraph = Graph(nGraph.vertices.filter(n => !nodesToRemove.contains(n._1)), nGraph.edges.filter(e => !(nodesToRemove.contains(e.srcId) || nodesToRemove.contains(e.dstId))))
      val delta = numberOfNodes - nGraph.vertices.count
      println("Number of removed nodes (MAX)" + nodesToRemove.length)
      println("Number of removed nodes (delta)" + delta)
      ...
      if (!terminate) {
        println("Shouldn't do this");
        // FIXME: this could crash if there are not enough edges
        var remainingEdges = sc.parallelize(nGraph.edges.takeOrdered(nGraph.edges.count.toInt - k)(Ordering[Double].reverse.on(_.attr)))
      nGraph = Graph(nGraph.vertices, remainingEdges)

    }
      println(nGraph.vertices.count)
      println("Number edges" + nGraph.edges.count)
    }
